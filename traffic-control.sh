...
function tc_incoming {
    ...
    hexAdd="$(printf '%02X' ${ADDRESS//./ }; echo)"
    testString=$(tc filter show dev $VIRTUAL | grep -B 1 -i "match $hexAdd")
    ...
    if [[ $(tc class show dev $VIRTUAL |grep "class htb 2:$UNIQID") ]]; then
        TCC="tc class change"
    else
    ...
    $TCC dev $VIRTUAL parent 2: classid 2:$UNIQID htb rate $SPEED_DOWNLOAD
    ...
    echo "- filter on IP $ADDRESS"
    TCF="tc filter add"
    if [[ $(tc filter show dev $VIRTUAL | grep "flowid 2:$UNIQID") ]]; then
        TCF="tc filter change"
    fi

    $TCF dev $VIRTUAL protocol ip parent 2: prio 1 u32 match ip src $ADDRESS flowid 2:$UNIQID
}
