...
path = ["statsJakarta", "statsHighway"]
modes = ["dds", "aws"]
# videos = ["uav-2"]
iteration = 0
for video_name in videos:
    for mode in modes:
        if mode == "dds":
            modeDir = ""
        dirName = "./profile-%s%s/profile-separated/" % (video_name, modeDir)
        ...
        # the profile should be separated, need a verification system
            ...
            stats = stats[stats["video-name"].str.match('^results/%s-5-%d_%s' % (video_name, segment, modeDir)) == True]
            # print(stats)
            ...
            #     currentConf += 1
            bandwidth_f1_remove_fluc.to_csv('./profile-%s%s/profile-separated/profile-%d.csv' %(video_name, modeDir, segment))
    iteration += 1